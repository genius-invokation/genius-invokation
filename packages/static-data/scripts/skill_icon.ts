...
const doFetch = async (id: number | string) => {
  ...
  const body = await fetch(url).then((r) => r.json() as Promise<any>);
...
export async function getSkillIcon(
  ...
  if (Reflect.has(mapping, iconHash)) {
    return mapping[iconHash];
  }
  console.log(
    `Skill id ${skillId} has unknown icon hash ${iconHash}, find it on Project Amber...`,
  );
