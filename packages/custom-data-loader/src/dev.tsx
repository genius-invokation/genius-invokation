...
`);
  ...
  const [assetsManager, setAssetsManager] = createSignal<AssetsManager | null>(null);
  ...
  const loadMod = () => {
    try {
      ...
      alert(`加载mod时出错：${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  return (
    <div class="app">
      ...
      <div class="tabs">
        ...
        <div 
          class={`tab ${activeTab() === 1 ? 'active' : ''} ${!step1Complete() ? 'disabled' : ''}`} 
          onClick={() => step1Complete() && setActiveTab(1)}
        >
          构建你的卡组
        </div>
        <div 
          class={`tab ${activeTab() === 2 ? 'active' : ''} ${!step2Complete() ? 'disabled' : ''}`} 
      ...
      <div class="content">
        ...
        <Show when={activeTab() === 1}>
          <div class="deck-buttons">
            <button onClick={() => setShowDeckBuilder1(true)}>
              编辑卡组1
            </button>
            <button onClick={() => setShowDeckBuilder2(true)}>
              编辑卡组2
            </button>
          </div>
          ...
          {/* 卡组1构建器对话框 */}
          <dialog open={showDeckBuilder1()} ref={(el) => {
          ...
          {/* 卡组2构建器对话框 */}
          <dialog open={showDeckBuilder2()} ref={(el) => {
