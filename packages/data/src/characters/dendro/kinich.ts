...
export const GrappleLink = status(117091)
  ...
  .callSnippet()
  // Trigger on other characters' technique before it resolves, so Grapple Link processes first
  .on("beforeSkill", (c, e) => e.isSkillType("technique") && e.techniqueCaller.id !== c.self.master.id)
  .listenToPlayer()
  .gainNightsoul("@master")
  .callSnippet()
  .on("beforeAction")
